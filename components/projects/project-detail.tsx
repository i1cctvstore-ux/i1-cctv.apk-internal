...
import { RegenerateLinkDialog } from '@/components/projects/regenerate-link-dialog'
import {
  formatDateTime,
  formatShortDate,
  initials,
...
export function ProjectDetail({ projectId, onBack }: ProjectDetailProps) {
  ...
  async function copyTrackingLink() {
    if (!project) return
    const url = `${window.location.origin}${trackingPath(project)}`
    await navigator.clipboard.writeText(url)
    setCopied(true)
    setTimeout(() => setCopied(false), 2000)
  }

  function handleLinkRegenerated(newToken: string) {
    setProject((prev) => (prev ? { ...prev, tracking_token: newToken } : prev))
  }
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <RegenerateLinkDialog
              ...
              onRegenerated={handleLinkRegenerated}
            />
          </div>
        </div>
      </div>
