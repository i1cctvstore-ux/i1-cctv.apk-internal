...
- Role yang tersedia: `super_admin`, `admin`, `kasir`, `gudang`, `teknisi`.
  Menu **User Role** cuma muncul dan bisa diakses oleh `super_admin` — role lain
  tidak melihat menunya sama sekali (dicek 3 lapis: disembunyikan di Sidebar,
  ditolak di API route, ditolak juga di RLS database).
...
(menu **User Role** setelah login sebagai Super Admin), langkah ini boleh dilewati.

> Catatan: menu **User Role** cuma bisa diakses `super_admin`, jadi pastikan
> minimal satu akun awal berperan `super_admin` (lewat seed atau lewat SQL
...
- Middleware sendiri **tidak mengecek role** — dia cuma pastikan sudah login.
  Pembatasan per-role dicek di 3 lapis terpisah (lihat `lib/nav-config.tsx`,
  `require-admin.ts`, dan RLS policy di `schema.sql`): menu **User Role**
  contohnya, cuma untuk `super_admin`. Menu lain (Dashboard, Stok, Servis)
  masih terbuka untuk semua role yang sudah login.
